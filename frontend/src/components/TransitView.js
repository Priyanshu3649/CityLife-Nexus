...
const TransitView = () => {
  ...
  const loadSupportedCities = async () => {
    try {
      const response = await fetch('http://localhost:8001/api/v1/routes/transit/cities');
      if (response.ok) {
  ...
  const searchTransitRoutes = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8001/api/v1/routes/transit/multimodal', {
        method: 'POST',
        headers: {
