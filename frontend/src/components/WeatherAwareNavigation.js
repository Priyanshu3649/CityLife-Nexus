...
const WeatherAwareNavigation = () => {
  ...
  const getWeatherRouteImpact = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8001/api/v1/routes/weather/route-impact?latitude=${mockCoordinates.from.latitude}&longitude=${mockCoordinates.from.longitude}&departure_time=${new Date().toISOString()}`, {
        method: 'GET',
        headers: {
